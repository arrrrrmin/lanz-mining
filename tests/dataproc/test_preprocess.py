...
from lanz_mining.dataproc import processors
...
def test_normalize_name_str():
    ...
    for name in names:
        res = processors.normalize_name_str(name)
...
def test_apply_policial_membership(dataframe: pl.DataFrame):
    ...
    _df = processors.apply_policial_membership(dataframe)
...
def test_apply_genre_affiliation(dataframe: pl.DataFrame):
    _df = processors.apply_policial_membership(dataframe)
    _df = processors.apply_group_affiliation(_df)
...
def test_apply_media_institute(dataframe: pl.DataFrame):
    dataframe = processors.apply_policial_membership(dataframe)
    dataframe = processors.apply_group_affiliation(dataframe)
    dataframe = processors.apply_media_institute(dataframe)
...
def test_known_keys_by_names(dataframe: pl.DataFrame):
    dataframe = processors.apply_policial_membership(dataframe)
    dataframe = processors.apply_group_affiliation(dataframe)
    name2group = processors.known_keys_by_names(dataframe, "group")
    assert all([value is not None for value in name2group.values()])
    dataframe = processors.apply_media_institute(dataframe)
    name2media = processors.known_keys_by_names(dataframe, "media")
...
def test_apply_nearest_entries(dataframe: pl.DataFrame):
    ...
    dataframe = processors.apply_group_affiliation(dataframe)
    name2group = processors.known_keys_by_names(dataframe, "group")
    dataframe = processors.apply_nearest_entries(dataframe, "group")
    all_groups = roles.Group.properties()
...
def test_apply_nearest_media(dataframe: pl.DataFrame):
    ...
    dataframe = processors.apply_group_affiliation(dataframe)
    dataframe = processors.apply_media_institute(dataframe)
    name2group = processors.known_keys_by_names(dataframe, "media")
    dataframe = processors.apply_nearest_entries(dataframe, "media")
